...
function renderSupplement(supplement) {
 ...
 const fields = [
   { label: '분류 ', value: supplement.types?.map(t => t.name).join(', ') || '-' },
   { label: '복용량 ', value: supplement.dosage || '-' },
   { label: '비용 ', value: supplement.cost || '-' },
   { label: '기대효과 ', value: supplement.effects?.map(e => e.effectTag?.name).join(', ') || '-' },
   { label: '부작용 ', value: supplement.sideEffects?.map(e => e.name).join(', ') || '-' },
 ];
